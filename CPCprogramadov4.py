...
def funcjulconfestproc():
    ...
    print('b) O julgamento parcial (art. 356) ou total (art. 355) do mérito;')
    print('c) O despacho saneador (art. 357).')

    while True:
        despachojulconfestproc = input('Insira a letra da opção que você deseja explorar: ')
        
        if despachojulconfestproc == 'a':
            print('A extinção do processo pode ocorrer em duas situações diferentes, quando não julgar o')
            print('mérito (art. 485), e quando julgar o mérito em decorrência de decadência ou prescrição,')
            print('(art. 487, II), ou no caso de homologação das possibilidades das alíneas do inciso III do')
            print('art, 487.')
        ...
        if despachojulconfestproc == 'b':
            print('em desenvolvimento')
            return despachojulconfestproc

        if despachojulconfestproc == 'c':
            print('em desenvolvimento')
            return despachojulconfestproc
